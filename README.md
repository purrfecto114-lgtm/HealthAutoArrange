...
- F8：打开 / 关闭设置窗口（内置 IMGUI）。在窗口里你可以：
  ...
  - 设置“同组内排序”：同一分组内按**当前效果强度**排列（默认从高到低，
    图标名末尾的数字；可切换为从低到高或按规则顺序），规则文件键 `InGroupSort`
  - 配置或预览提醒（reminders）
  - 切换界面语言（右上角 EN / 中文）
...
- v1.2.3 起，同一分组内默认按**当前效果强度从高到低**排序（同强度回退规则顺序，
  未知强度排最后；组间顺序不受影响）。旧配置无 `InGroupSort` 键时自动应用；
  要恢复 v1.2.2 行为请设 `InGroupSort = RuleIndex`。
- v1.2.3 渲染修复：重建帧即时隐藏待销毁旧图标（消除“旧+新排列重叠一帧”的闪现），
  新状态创建帧预置正确淡入颜色（消除“先不透明闪一帧再淡入”）；关闭自动整理时
  完全还原原版渲染行为。
- v1.2.3 起看门狗把“重建后的预期收尾扫描”记录为 Debug 级（不再以 Warning 刷屏），
  仅真正意外的位置漂移才告警；F9 诊断输出与当前管线一致（组名/强度/槽位/当前统计，
  捕获元数据不再因刷新边界而显示 unknown）。
