...
namespace HealthAutoArrange.Core
{
    ...
    public sealed class ReminderPresentationItem
    {
        ...
        /// <summary>是否为预览项（Preview 只替换旧预览，不影响同状态正式提醒）。</summary>
    ...
    public sealed class ReminderPresentation
    {
            ...
            string template = null)
        {
            ...
            // A frequent repeating reminder should refresh its own visual slot rather than stacking
            // several copies of the same state on screen. Other states still queue independently.
            ...
            string template = null)
        {
            ...
            var text = ReminderTemplateFormatter.Render(template ?? _defaultTemplate, context);
            _items.RemoveAll(i => i.IsPreview
                && string.Equals(i.BaseState, baseState, StringComparison.OrdinalIgnoreCase));
            var item = new ReminderPresentationItem(baseState, text, activePreset, now, isPreview: true);
            _items.Add(item);
            return item;
        }
        ...
        {
            // Reminder rules generated by the UI are wildcard patterns. PatternBaseId preserves
            // meaningful trailing digits in third-party IDs (for example "drug2#" or legacy "drug2*") instead of
            // misreading them as a vanilla severity suffix.
            return MoodleIdentity.PatternBaseId(state);
        }
    }
}