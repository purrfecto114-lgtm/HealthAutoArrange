...
# v1.1.6 UX / updater regression contracts
updater=text('HealthAutoArrange.Plugin/SafeUpdater.cs')
release=text('.github/workflows/release.yml')
check('v1.1.6 metadata is active', '"1.1.6"' in plugin and '1.1.6.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs'))
...
check('updater never hot-replaces plugin DLLs', 'File.Move(temporary, destination)' in updater and 'BepInEx.Paths.ConfigPath' in plugin and 'Paths.PluginPath' not in updater and 'Assembly.Load' not in updater)
check('updater staging failures cannot leave busy state stuck', 'Could not prepare the update staging directory:' in updater and '_busy = false;' in updater)
check('updater chooses highest verified manifest after all sources finish', 'if (allDone) break;' in updater and 'if (firstValid != null)' in updater and 'AcceptManifest(parsed);' not in updater.split('while (true)',1)[1].split('if (firstValid != null)',1)[0])
check('updater UI reports download progress', 'snapshot.Progress01 * 100f' in window and 'DownloadProgress' in textcat)
check('updater leaves Unity-managed User-Agent untouched', 'User-Agent' not in updater and 'SetRequestHeader' not in updater)
check('updater rejects HTTPS-to-non-HTTPS redirect results', '&& IsHttpsUrl(request.url);' in updater)
check('update manifest requires pinned RSA-SHA256 signature', 'PublicModulusBase64' in updater and 'VerifyHash' in updater and 'CryptoConfig.MapNameToOID("SHA256")' in updater)
check('download requires signed size and SHA-256 match', 'info.Length != manifest.Size' in updater and 'SHA256.Create()' in updater and 'manifest.Sha256' in updater)
check('updater aborts oversized network responses before post-download verification', 'MaxManifestBytes = 32UL * 1024UL' in updater and 'request.Abort();' in updater and 'request.downloadedBytes > (ulong)MaxPackageBytes' in updater)
check('updater rejects unsafe asset paths and non-HTTPS manifest URLs', 'IsSafeAssetName' in updater and "value.IndexOf('\\\\') < 0" in updater and 'IsHttpsUrl' in updater)
check('official and CDN manifest sources are both supported', 'raw.githubusercontent.com' in plugin and 'cdn.jsdelivr.net' in plugin and 'AllowMirror' in plugin)
check('custom signed mirror can serve its own package bytes', 'DeriveTransportPackageUrl' in updater and 'manifest.SourcePackageUrl' in updater and 'packages/' in updater)
check('updater checks sources without blocking game main loop', 'StartCoroutine' in updater and 'SendWebRequest()' in updater and 'yield return null;' in updater)
check('release workflow signs manifest and publishes mirror branch', 'UPDATE_SIGNING_PRIVATE_KEY_PEM' in release and 'SignData' in release and 'update-dist' in release and 'Get-FileHash -Algorithm SHA256' in release)
check('private signing key patterns are gitignored', 'update-signing-private' in text('.gitignore'))
