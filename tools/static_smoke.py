...
check('csproj falls back to committed build refs when GameDir is unset', "$(MSBuildThisFileDirectory)..\\refs" in csproj)
check('committed build refs directory exists with BepInEx core DLLs', 'refs/BepInEx/core/BepInEx.dll' in text('refs/README.md') or __import__('os').path.exists('refs/BepInEx/core/BepInEx.dll'))
check('committed build refs directory exists with managed stubs', __import__('os').path.exists('refs/CasualtiesUnknown_Data/Managed/Assembly-CSharp.dll'))
...
check('game refresh path applies sort same-frame to avoid UI flicker regression', '_scheduler.TryRunNow()' in adapter and 'ProcessRefresh();' in adapter.split('public void OnMoodlesUpdated',1)[1].split('public void Update',1)[0])
check('topology drift and nested refresh still use frame-aware scheduler gate', '_scheduler.OnGameRefreshCompleted(Time.frameCount)' in adapter and '_scheduler.TryDeferred(Time.frameCount)' in adapter and 'ScheduleAfterCurrentFrame();' in adapter.split('private bool ApplySiblingOrder',1)[1].split('private bool ApplyAnchoredSlots',1)[0])
...
check('refresh hook resolves exact parameterless signature (all v1.2.2 boundaries)', 'AccessTools.Method(typeof(MoodleManager), boundary.Target, Type.EmptyTypes)' in plugin and '"UpdateMoodles"' in plugin and '"AddAllMoodles"' in plugin and '"ClearMoodles"' in plugin)
...
check('current release metadata is active', ('"1.1.8"' in plugin or '"1.1.9"' in plugin or '"1.2.0"' in plugin or '"1.2.1"' in plugin or '"1.2.2"' in plugin or '"1.2.3"' in plugin) and ('1.1.8.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs') or '1.1.9.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs') or '1.2.0.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs') or '1.2.1.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs') or '1.2.2.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs') or '1.2.3.0' in text('HealthAutoArrange.Plugin/Properties/AssemblyInfo.cs')))
...
check('save applies before persistence to avoid rejected on-disk config', 'if (!ApplyModel(model))' in plugin and plugin.index('if (!ApplyModel(model))') < plugin.index('RulesFileStore.Write(_rulesPath, model)'))
check('update availability is visible outside collapsed section', 'headerUpdate.HasUpdate' in window and 'UpdateBadge' in window and '_showUpdates = true;' in window)
check('tooltip overlay stays away from fixed footer controls', 'GUI.Box(new Rect(12f, 32f, width, height)' in window)
check('updater is notification-only and never downloads packages', 'DownloadHandlerFile' not in updater and 'DownloadAvailable' not in updater and 'DownloadUpdate' not in plugin)
check('updater is triggered exactly once from plugin startup', plugin.count('AutoCheckAfterDelay(') == 1 and 'CheckForUpdates' not in plugin and 'Check now' not in textcat)
check('available update produces one visible in-game reminder', 'onUpdateAvailable' in updater and 'ShowUpdateAvailableFeedback' in plugin and 'camera.DoAlert(text, false);' in plugin)
check('updater leaves Unity-managed User-Agent untouched', 'User-Agent' not in updater and 'SetRequestHeader' not in updater)
check('updater rejects redirects outside allowed manifest hosts', '&& IsAllowedManifestUrl(request.url);' in updater)
check('updater does not embed RSA trust material (removed in v1.1.8)', 'PublicModulusBase64' not in updater and 'VerifyHash' not in updater and 'RSACryptoServiceProvider' not in updater and 'CryptoConfig.MapNameToOID' not in updater)
check('updater manifest schema is 2 (no signature field)', 'ManifestSchema = 2' in updater and 'TryParseManifest' in updater and 'TryParseAndVerifyManifest' not in updater)
check('updater aborts oversized manifest responses', 'MaxManifestBytes = 32UL * 1024UL' in updater and 'request.Abort();' in updater)
check('update sources are GitHub + jsDelivr manifest mirrors (v1.2.2)', 'raw.githubusercontent.com' in plugin and 'cdn.jsdelivr.net' in plugin and 'github.com' in updater)
check('updater checks GitHub without blocking game main loop', 'StartCoroutine(_updater.AutoCheckAfterDelay' in plugin and 'SendWebRequest()' in updater and 'yield return null;' in updater)
check('release workflow does not require signing secrets (v1.1.8+)', 'UPDATE_SIGNING_PRIVATE_KEY_PEM' not in release and 'SignData' not in release)
check('release workflow publishes unsigned schema-2 manifest', 'update-dist' in release and 'schema=2' in release)
check('release workflow does not require GAME_REFS_URL secret (committed refs fallback)', 'GAME_REFS_URL not configured' in release and 'Using committed build refs' in release)
check('gitignore blocks PEM defensively', '*.pem' in text('.gitignore') and 'update-signing-private' not in text('.gitignore'))
